...
elseif ($action == 'address_list')
{
    ...
    if (count($consignee_list) < 2 && $_SESSION['user_id'] > 0)
    {
        /* 如果用户收货人信息的总数小于2 则增加一个新的收货人信息 */
...
elseif ($action == 'collection_list')
{
 ...
 $page = isset($_REQUEST['page']) ? intval($_REQUEST['page']) : 1;
 $andsql=$_REQUEST['catid']?" AND g.cat_id=$_REQUEST[catid]":"";
 $andsql.=isset($_REQUEST['type1'])? " AND g.is_on_sale='1'":"";
 $andsql.=isset($_REQUEST['type2'])? " AND g.is_best='1'":"";
 $andsql.=isset($_REQUEST['type3'])? " AND g.is_new='1'":"";
 $andsql.=isset($_REQUEST['type4'])? " AND g.is_hot='1'":"";
 $time = gmtime();
 $andsql.=isset($_REQUEST['type5'])? " AND g.is_promote='1' AND promote_start_date <= '$time' AND promote_end_date >= '$time'":"";
 
    $record_count = $db->getOne("SELECT COUNT(*) FROM " .$ecs->table('collect_goods')." AS cg ".
        "LEFT JOIN ".$ecs->table('goods')." AS g ".
        ' ON cg.goods_id=g.goods_id '.
        "LEFT JOIN ".$ecs->table('category')." AS c ".
        ' ON g.cat_id=c.cat_id '.
                                " WHERE cg.user_id='$user_id' $andsql ORDER BY cg.add_time DESC");
    $total = $db->getOne("SELECT COUNT(*) FROM " .$ecs->table('collect_goods')." AS cg ".
                                " WHERE cg.user_id='$user_id' ORDER BY cg.add_time DESC");

 
    $pager = get_pager('user.php', array('act' => $action), $record_count, $page);
    $smarty->assign('pager', $pager);
    $smarty->assign('goods_list', get_collection_goods($user_id, $pager['size'], $pager['start'],$_REQUEST['catid'],$_REQUEST['type1'],$_REQUEST['type2'],$_REQUEST['type3'],$_REQUEST['type4'],$_REQUEST['type5']));
    $smarty->assign('goods_cate',get_goods_cate($user_id));
    $smarty->assign('goods_all',$total);
    $lang_list = array(

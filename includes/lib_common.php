...
{   
    ...
    $sql = 'SELECT topic_id, title,topic_img,description,keywords FROM ' . $GLOBALS['ecs']->table('topic') . " WHERE data=0 AND start_time <= '$gmtime' AND end_time >= '$gmtime'";
...
{    
	...
	$arr[$idx]['id'] = $row['topic_id'];
	$arr[$idx]['title'] = $row['title'];
	$arr[$idx]['topic_img'] = $row['topic_img'];
...
{
    $sql = 'SELECT a.article_id, a.title, ac.cat_name, a.add_time,a.file_url,a.article_type, a.open_type, ac.cat_id, ac.cat_name ' .

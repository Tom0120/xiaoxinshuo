 ...
 * @return  array   $arr
 */
function get_collection_goods($user_id, $num = 10, $start = 0,$cat_id='',$a='',$b='',$c='',$d='',$e='')
{ 
 $andsql=$cat_id?" AND g.cat_id=$cat_id":"";
 $andsql.=$a?" AND g.is_on_sale='1'":"";
 $andsql.=$b?" AND g.is_best='1'":"";
 $andsql.=$c?" AND g.is_new='1'":"";
 $andsql.=$d?" AND g.is_hot='1'":"";
 $time = gmtime();
 $andsql.=$e?" AND g.is_promote='1' AND promote_start_date <= '$time' AND promote_end_date >= '$time'":"";
 
    $sql = 'SELECT g.goods_id, g.goods_name, g.market_price, g.shop_price AS org_price,g.goods_thumb,c.add_time, '.
   ...
   ' ON g.cat_id=gc.cat_id '.
   " WHERE 1=1 ".
   $andsql.
   " AND c.user_id = '$user_id' ORDER BY c.rec_id DESC";
  ...
  {
  /*获得折数*/
  $goods_list[$row['goods_id']]['zheshu'] = number_format($row['promote_price']/$row['market_price'],2);

  /*获得差价*/
  $goods_list[$row['goods_id']]['chajia']  = price_format($row['market_price']-$row['promote_price']);  
  }
  else
  {
  /*获得折数*/
  $goods_list[$row['goods_id']]['zheshu'] = number_format($row['shop_price']/$row['market_price'],2);

  /*获得差价*/
  $goods_list[$row['goods_id']]['chajia']  = price_format($row['market_price']-$row['shop_price']);  
  }

        $goods_list[$row['goods_id']]['rec_id']        = $row['rec_id'];
        $goods_list[$row['goods_id']]['goods_thumb']        = $row['goods_thumb'];
        $goods_list[$row['goods_id']]['add_time']        = local_date($GLOBALS['_CFG']['date_format'], $row['add_time']);
...
//商品关注
function get_goods_cate($user_id)
{
    $sql = 'SELECT  count(g.goods_id) as total, g.cat_id,ec.cat_name ' .
            ' FROM ' . $GLOBALS['ecs']->table('collect_goods') . ' AS c' .
            " LEFT JOIN " . $GLOBALS['ecs']->table('goods') . " AS g ".
                "ON g.goods_id = c.goods_id ".
   " LEFT JOIN " . $GLOBALS['ecs']->table('category'). " AS ec  ".
   " ON ec.cat_id=g.cat_id ".
            " WHERE c.user_id = '$user_id'".
   ' GROUP BY g.cat_id  ORDER BY c.rec_id DESC';
 $res = $GLOBALS['db']->query($sql);

    $goods_list = array();
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
  $goods_list[$row['cat_id']]=$row;
    }
    return $goods_list;
}
/**
...
function get_user_default($user_id)
{
    ...
    $sql = "SELECT pay_points,qq, mobile_phone,user_money, credit_line, last_login, is_validated FROM " .$GLOBALS['ecs']->table('users'). " WHERE user_id = '$user_id'";
    $row = $GLOBALS['db']->getRow($sql);
    $info = array();
    $info['username']  = stripslashes($_SESSION['user_name']);
	$info['email']  = stripslashes($_SESSION['email']);
    ...
    $info['qqs']  = $row['qq'];
	$info['mobilephone']  = $row['mobile_phone'];
	...
	//获取用户商品收藏数
    $collect_sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('collect_goods').
            " WHERE user_id = '" .$user_id. "'";
    $info['collect_count'] = $GLOBALS['db']->getOne($collect_sql);

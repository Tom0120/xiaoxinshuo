...
function get_categories_tree($cat_id = 0)
{
    ...
    */
    ...
    {
        /* 获取当前分类及其子分类 */
        $sql = 'SELECT cat_id,cat_name,cat_img,parent_id,is_show ' .
...
function get_recommend_goods($type = '', $cats = '')
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        $sql = 'SELECT b.brand_id,b.brand_name,b.brand_logo,b.brand_desc,b.site_url,g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
                "promote_start_date, promote_end_date,g.click_count, g.goods_brief, g.goods_thumb, g.goods_img, RAND() AS rnd " .
                'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
			 'LEFT JOIN ' . $GLOBALS['ecs']->table('brand') . ' AS b ON b.brand_id = g.brand_id ' .
        ...
        foreach ($result AS $idx => $row)
        {
			...
			$goods[$idx]['count']           = $row['click_count'];

            $goods[$idx]['name']         = $row['goods_name'];
            $goods[$idx]['brief']        = $row['goods_brief'];
			$goods[$idx]['brand_name']        = $row['brand_name'];//调用推荐商品品牌
			$goods[$idx]['brand_logo']        = $row['brand_logo'];//调用推荐商品品牌
			$goods[$idx]['brand_id']        = $row['brand_id'];//调用推荐商品品牌
 ...
 * return $res
 */
function get_level2_category($cat_id)
{
...
* @return array 
*/ 
function index_get_class_articless($cat_aid, $cat_num) 
{ 
$sql = "SELECT article_id, title,open_type,link,description,click_count,content,cat_id,goods_link,file_url FROM " .$GLOBALS['ecs']->table('article'). " WHERE cat_id = ".$cat_aid." and is_open = 1 LIMIT " . $cat_num; 
$res = $GLOBALS['db']->getAll($sql); 
$arr = array(); 
foreach ($res AS $idx => $row) 
{ 
...
$arr[$idx]['click_count'] = $row['click_count'];
$arr[$idx]['content'] = $row['content'];
$arr[$idx]['goods_link'] = $row['goods_link'];

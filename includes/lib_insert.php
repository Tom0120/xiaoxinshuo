...
function insert_history()
{
    ...
    if (!empty($_COOKIE['ECS']['history']))
    {
        ...
        while ($row = $GLOBALS['db']->fetch_array($query))
        {
            ...
            $str.='<ul id="buy">
			<li>
			<a class="img" href="'.$goods['url'].'" target="_blank"><img src="'.$goods['goods_thumb'].'" alt="'.$goods['goods_name'].'" /></a>
			<a style="margin-top:-10px;" class="name" href="'.$goods['url'].'" target="_blank" title="'.$goods['goods_name'].'">'.$goods['short_name'].'</a> <p class="price" style="margin-top:-17px;">'.$goods['shop_price'].'</p></li></ul>';
        }
...
function insert_kefushow($arr)
{
	...
	$val = $GLOBALS['smarty']->fetch('library/xinyu_kefu_show.lbi');
	return $val;
}

    ...
    if ($filename != 'index')
    {
        ...
        if (in_array($filename, array('category', 'goods', 'article_cat', 'article', 'brand')))
        {
            ...
            if (!empty($cat_arr))
            {
                ...
                foreach ($cat_arr AS $val)
                {
                    ...
                    $ur_here   .= ' <i class="arrow"></i> <a href="' . build_uri($type, $args, $val['cat_name']) . '">' .
        ...
        else
        {
            ...
            if ('group_buy' == $filename)
            {
                ...
                $args       = array('gbid' => '0');
                $ur_here   .= ' <i class="arrow"></i> <a href="group_buy.php">' .
...
function build_urhere($arr, $type = 'category')
{
    ...
    foreach ($arr AS $val)
    {
        ...
        $str .= ' <i class="arrow"></i> <a href="' . build_uri($type, $args). '">' . htmlspecialchars($val['cat_name']) . '</a>';
    }
...
function get_shop_help()
{
    ...
    foreach ($res AS $key => $row)
    {
		...
		$arr[$row['cat_id']]['cat_idx']= $row['cat_id'];  //用户数据文章分类的id

...
function reformat_image_name($type, $goods_id, $source_img, $position='')
{
    ...
    if (!make_dir(ROOT_PATH.$dir.'/'.$sub_dir.'/goods_img'))
    {
	 ...
	 if (!make_dir(ROOT_PATH.$dir.'/'.$sub_dir.'/goods_ico'))
    {
        return false;
    }
    ...
    {
        case 'goods':
            $img_name = $goods_id . '_N_' . $rand_name;
            break;
		case 'goods_new':
    ...
    {
        if (move_image_file(ROOT_PATH.$source_img, ROOT_PATH.$dir.'/'.$sub_dir.'/goods_ico/'.$img_name.$img_ext))
        {
            return $dir.'/'.$sub_dir.'/goods_ico/'.$img_name.$img_ext;
        }
    }

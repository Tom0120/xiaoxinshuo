...
if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'trash')
{
    ...
    //商品雷达改动by yujian 2012-4-20
    if($_CFG['enable_radar'] == 1 && $_REQUEST['act'] == 'list'){
    	include_once("radar_sign.php");
        $sql = "SELECT value FROM " . $ecs->table('shop_config') . " WHERE code = 'certificate_id'";
        $radar_license = $db->getOne($sql);    
        $smarty->assign('license', $radar_license);
        $smarty->assign('product_key', "ecshop");
        $smarty->assign('radar_sign', getRadarSign());
        
        foreach ($goods_list['goods'] as $k=>$v){
        	$sql = "SELECT b.brand_name FROM " . $ecs->table('goods') . " g 
        	left join " . $ecs->table('brand') . " b ON b.brand_id=g.brand_id WHERE g.goods_id = '".$v['goods_id']."'";
        	$goods_list['goods'][$k]['brand_name'] = $db->getOne($sql);
        	$sql = "SELECT c.cat_name FROM " . $ecs->table('goods') . " g 
        	left join " . $ecs->table('category') . " c ON c.cat_id=g.cat_id WHERE g.goods_id = '".$v['goods_id']."'";
        	$goods_list['goods'][$k]['cat_name'] = $db->getOne($sql);
        }
        $smarty->assign('goods_list',   $goods_list['goods']);
    	$smarty->display("radar_goods_list.htm");
    }else{
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if (isset($_FILES['goods_img']['error'])) // php 4.2 版本才支持 error
    {
		...
		elseif ($_FILES['goods_ico']['error'] == 1)
        {
        ...
        elseif ($_FILES['goods_img']['error'] == 2)
        {
         ...
         if (isset($_FILES['goods_ico']['error'])) // php 4.2 版本才支持 error
       {
        // 最大上传文件大小
        $php_maxsize = ini_get('upload_max_filesize');
        $htm_maxsize = '2M';
        ...
        // 商品图片
        if ($_FILES['goods_ico']['error'] == 0)
        {
            if (!$image->check_img_type($_FILES['goods_ico']['type']))
            {
                sys_msg($_LANG['invalid_goods_img'], 1, array(), false);
            }
        }
        ...
        elseif ($_FILES['goods_ico']['error'] == 2)
        {
            sys_msg(sprintf($_LANG['goods_img_too_big'], $htm_maxsize), 1, array(), false);
        }
    ...
    $old_original_img = '';  // 初始化原始图片旧图
	 ...
	 // 是否上传商品图片new
    if (isset($_FILES['goods_ico']) && $_FILES['goods_ico']['tmp_name'] != '' &&
            isset($_FILES['goods_ico']['tmp_name']) &&$_FILES['goods_ico']['tmp_name'] != 'none')
    {
        // 上传了，直接使用，原始大小
        $goods_ico = $image->upload_image($_FILES['goods_ico']);
        if ($goods_ico === false)
        {
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "promote_start_date, promote_end_date, goods_img, goods_thumb,goods_ico, original_img, keywords, goods_brief, " .
                ...
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    ...
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb','$goods_ico', '$original_img', ".
    ...
    {
        /* 如果有上传图片，删除原来的商品图 */
        $sql = "SELECT goods_thumb,goods_ico, goods_img, original_img " .
                    " FROM " . $ecs->table('goods') .
                    " WHERE goods_id = '$_REQUEST[goods_id]'";
        $row = $db->getRow($sql);
		
        if ($proc_thumb && $goods_img && $row['goods_ico'] && !goods_parse_url($row['goods_ico']))
        {
            @unlink(ROOT_PATH . $row['goods_ico']);
        }
	...
	$goods_ico = reformat_image_name('goods_new', $goods_id, $goods_ico,  'goods_new');//新的图片
    if ($goods_img !== false)
    {
     ...
     if ($goods_ico !== false)
    {
        $db->query("UPDATE " . $ecs->table('goods') . " SET goods_ico = '$goods_ico' WHERE goods_id='$goods_id'");
    }
    if ($original_img !== false)
    {
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    //商品雷达改动by yujian 2012-4-20
    if($_CFG['enable_radar'] == 1 && !$is_delete){
		include_once("radar_sign.php");
        $sql = "SELECT value FROM " . $ecs->table('shop_config') . " WHERE code = 'certificate_id'";
        $radar_license = $db->getOne($sql);    
        $smarty->assign('license', $radar_license);

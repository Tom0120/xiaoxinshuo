...
if (!$smarty->is_cached('survey.dwt', $cache_id))
{
    assign_template();
    ...
    $smarty->assign("banner",getads(2,1));//1表示广告位置的id，4表示调用的数量。
    $smarty->assign("banner2",getads(3,2));
	$smarty->assign("banner4",getads(4,1));

    $smarty->assign('brand_list',      get_brands());
    $smarty->assign('promotion_info',  get_promotion_info()); // 增加一个动态显示所有促销信息的标签栏
    $smarty->assign('class_articles_22', index_get_class_articless(22,5)); // 分类调用文章 
...
$smarty->display('survey.dwt', $cache_id);

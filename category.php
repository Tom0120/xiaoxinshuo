...
if (!$smarty->is_cached('category.dwt', $cache_id))
{
    ...
    if (!empty($cat))
    {
        ...
        $smarty->assign('catid',   htmlspecialchars($cat['cat_id']));//调用当前分类id
        $smarty->assign('catname',   htmlspecialchars($cat['cat_name']));//调用当前分类name
    ...
    $smarty->assign('level_category2',     get_level2_category($cat_id));  // 获取子级分类
...
{
    return $GLOBALS['db']->getRow('SELECT cat_id,cat_name, keywords, cat_desc, style, grade, filter_attr, parent_id FROM ' . $GLOBALS['ecs']->table('category') .
        " WHERE cat_id = '$cat_id'");
}
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style,g.click_count,g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
		        ...
		        " IFNULL(AVG(r.comment_rank),0) AS comment_rank,IF(r.comment_rank,count(*),0) AS comment_count, ".//调用商品评论
                'g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb , g.goods_img ' .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' .
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
				' LEFT JOIN '. $GLOBALS['ecs']->table('comment') .' AS r '.
'ON r.id_value = g.goods_id AND comment_type = 0 AND r.parent_id = 0 AND r.status = 1 ' .//调用评论
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$arr[$row['goods_id']]['count']             = $row['click_count'];
        $arr[$row['goods_id']]['goods_brief']      = $row['goods_brief'];
		$arr[$row['goods_id']]['comment_count']=$row['comment_count'];//调用评论
        $arr[$row['goods_id']]['buy_num'] = get_buy_sum($row['goods_id']);//调用销量
